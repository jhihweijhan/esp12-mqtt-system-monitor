...
void test_sender_topic_validation_policy() {
    TEST_ASSERT_TRUE(isValidSenderMetricsTopic("sys/agents/desk/metrics/v2"));
    TEST_ASSERT_TRUE(isValidSenderMetricsTopic("sys/agents/nas-01/metrics/v2"));

    TEST_ASSERT_FALSE(isValidSenderMetricsTopic(nullptr));
    TEST_ASSERT_FALSE(isValidSenderMetricsTopic(""));
    TEST_ASSERT_FALSE(isValidSenderMetricsTopic("sys/agents/+/metrics/v2"));
    TEST_ASSERT_FALSE(isValidSenderMetricsTopic("sys/agents/#/metrics/v2"));
    TEST_ASSERT_FALSE(isValidSenderMetricsTopic("sys/agents//metrics/v2"));
    TEST_ASSERT_FALSE(isValidSenderMetricsTopic("sys/agents/desk/metrics"));
...
void test_sender_topic_hostname_extract_policy() {
    ...
    TEST_ASSERT_EQUAL_STRING("desk", host);

    TEST_ASSERT_TRUE(extractHostnameFromSenderTopic("sys/agents/workstation-01/metrics/v2", host, sizeof(host)));
    TEST_ASSERT_EQUAL_STRING("workstation-01", host);

    TEST_ASSERT_FALSE(extractHostnameFromSenderTopic("sys/agents//metrics/v2", host, sizeof(host)));
    TEST_ASSERT_FALSE(extractHostnameFromSenderTopic("sys/agents/desk/metrics", host, sizeof(host)));
}
